...
class CommentsSheet extends StatefulWidget {
  ...
  final int roomAuthorId;

  const CommentsSheet({
    super.key,
    required this.roomId,
    required this.roomTitle,
    required this.roomAuthorId,
  });
...
class _CommentsSheetState extends State<CommentsSheet> {
  ...
  bool get _isRoomOwner {
    ...
    return currentId != null && currentId == widget.roomAuthorId;
  }
  ...
  Future<void> _loadReplies(int commentIndex) async {
    ...
    final result = await _commentService.getReplies(
      comment['id'] as int,
      page: nextPage,
    );
  ...
  Future<void> _toggleCommentLike(int commentId, int index) async {
    ...
    if (result != null && mounted) {
      setState(() {
        _comments[index]['is_liked'] = result['is_liked'];
        _comments[index]['likes_count'] = result['likes_count'];
      });
    }
  }
  ...
  Widget _buildCommentItem(Map<String, dynamic> comment, int index) {
    ...
    final canDelete = isOwnComment || _isRoomOwner;
        ...
        children: [
          ...
          if (replies.isNotEmpty) ...[
            ...
            ...replies.asMap().entries.map(
              (entry) => _buildReplyPreview(entry.value, index),
            ),
          ],
  ...
  Future<void> _toggleReplyLike(
    ...
    final result = await _commentService.toggleCommentLike(reply['id']);

    if (result != null && mounted) {
      setState(() {
        reply['is_liked'] = result['is_liked'];
        reply['likes_count'] = result['likes_count'];

        final comment = _comments[commentIndex];
        final replies = comment['replies'] as List;
        final replyIndex = replies.indexWhere((r) => r['id'] == reply['id']);
        if (replyIndex != -1) {
          replies[replyIndex]['is_liked'] = result['is_liked'];
          replies[replyIndex]['likes_count'] = result['likes_count'];
        }
      });
    }
  }
  ...
  Widget _buildReplyPreview(Map<String, dynamic> reply, int commentIndex) {
    ...
    final canDelete = isOwnReply || _isRoomOwner;
                ...
                child: Column(
                  ...
                  children: [
                        ...
                        children: [
                          ...
                          if (replyToUser != null) ...[
                            WidgetSpan(
                              ...
                              child: GestureDetector(
                                onTap: () =>
                                    _navigateToUserProfile(replyToUserId),
                                child: Text(
                    ...
                    Row(
                      children: [
                        InkWell(
                          onTap: () => _toggleReplyLike(reply, commentIndex),
                          child: Row(
                            children: [
                              Icon(
                                isLiked
